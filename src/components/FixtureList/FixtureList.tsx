...
export function FixtureList({ sportSlug }: FixtureListProps) {
  const { sportFilterType, fixtureSearchQuery } = useUiStore();
  const effectiveType = sportFilterType;
  ...
  const displayTitle = sportSlug.replace(/-/g, ' ');
  ...
  if (!fixtures.length) {
    return (
      <div className="flex-1 min-h-0 p-8 flex flex-col" style={{ background: 'var(--app-bg-deep)' }}>
        <div className="flex justify-between items-center w-full max-w-4xl mx-auto mb-6">
          ...
          <div className="live-badge flex items-center gap-2 px-3 py-1.5 rounded-full">
            <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: effectiveType === 'live' ? 'var(--app-error)' : 'var(--app-accent)' }} />
            <span className="text-xs uppercase font-bold tracking-wider">{effectiveType === 'live' ? 'Live' : 'Upcoming'}</span>
          </div>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center" style={{ color: 'var(--app-text-muted)' }}>
          ...
          <span className="font-semibold text-base">No fixtures for {displayTitle}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin" style={{ background: 'var(--app-bg-deep)', scrollbarColor: 'var(--app-border) transparent' }}>
      <div className="sports-header sticky top-0 z-10 px-3 md:px-4 py-3">
        <div className="w-full flex justify-between items-center">
          ...
          <div className="live-badge flex items-center gap-2 px-3 py-1.5 rounded-full">
            <span className="h-2 w-2 rounded-full animate-pulse" style={{ background: isLive ? 'var(--app-error)' : 'var(--app-accent)' }} />
            <span className="text-xs uppercase font-bold tracking-wider">{isLive ? 'Live Feed' : 'Upcoming Feed'}</span>
          </div>
        </div>
      </div>
