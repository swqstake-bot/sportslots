...
export function attachHunterHouseBetCoordinator(ctx) {
  ...
  const runWorkerTick = () => {
    ...
    if (houseBetRetryBufferRef?.current) {
    ...
    for (const bItem of batch) {
      ...
      if (p) {
        ...
        if (directProviderBetId && p.feedEntryId) {

          betShareIdRegistry.register({

            providerBetId: directProviderBetId,

            feedEntryId: p.feedEntryId,
      ...
      if (!p && hasRunningForHouseBet) {
        ...
        if (dedupeKey && houseBetRetryBufferRef?.current) {
  ...
  return () => {
    ...
    if (houseBetRetryBufferRef?.current) houseBetRetryBufferRef.current = []
