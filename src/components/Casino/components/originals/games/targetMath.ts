...
/** Stake Limbo: multiplierTarget up to 1_000_000×. */
...
/** Clamp to Stake Dice multiplier range (default max 9900×). */
...
/** Clamp to Stake Limbo multiplier range (1.01×–1_000_000×). */
export function clampLimboMultiplier(n: number): number {
  return Math.min(
    LIMBO_MAX_MULTIPLIER,
    Math.max(LIMBO_MIN_MULTIPLIER, Number(n) || LIMBO_MIN_MULTIPLIER)
  )
}
