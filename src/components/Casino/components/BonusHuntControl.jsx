...
import { formatAmount, formatBetLabel, isFiat, isStable } from '../utils/formatAmount'
...
export default function BonusHuntControl({
  ...
  useEffect(() => {
    if (cryptoOnly) {
      if (isFiat(sourceCurrency) && !isStable(sourceCurrency)) {
        const firstCrypto = cryptoOpts[0]?.value
        if (firstCrypto) setSourceCurrency(firstCrypto)
      }
      if (isFiat(targetCurrency) && !isStable(targetCurrency)) {
        const firstCrypto = cryptoOpts[0]?.value
        if (firstCrypto) setTargetCurrency(firstCrypto)
      }
    }
  }, [cryptoOnly, sourceCurrency, targetCurrency, cryptoOpts])
      ...
      while (!cancelRef.current && !gotBonus) {
        ...
        try {
          ...
          setBetHistory((h) => [
            ...h,
            {
              id: `${slot.slug}-${Date.now()}-${slotSpins}`,
              slotSlug: slot.slug,
              slotName: slot.name,
  ...
  const format = (v) => {
    const f = formatAmount(v, targetCurrency)
  ...
  const getDisplayWin = (b) => (b.isBonus && b.stoppedBonus ? 0 : (b.winAmount ?? 0))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', minWidth: 0, color: 'var(--text)' }}>
        ...
        <div style={STYLES.row}>
          ...
          >
            ...
            {(!cryptoOnly && fiatOpts.length > 0) && <optgroup label="Fiat">{fiatOpts.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}</optgroup>}
          </select>
        ...
        <div style={{ ...STYLES.row, marginTop: '0.75rem', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem' }} title="Hacksaw: Bei 4+ werden nur 4/5-Scatter-Boni gesammelt (Stopp). 3er-Boni werden durchgespielt und weiter gejagt.">
            Stopp bei Scatter:
            <select
      ...
      {(currentBalance != null || isRunning) && (
        <div style={{ marginTop: '1rem', marginBottom: '0.5rem' }}>
          ...
          <div style={STYLES.balanceBadge}>
            {currentBalance != null ? formatAmount(currentBalance, currencyCode || targetCurrency) : '–'}
            {(currencyCode || targetCurrency) && (
              <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>{(currencyCode || targetCurrency).toUpperCase()}</span>
            )}
          </div>
        </div>
      )}
      ...
      {betHistory.length > 0 && (() => {
        ...
        if (slotKeys.length === 1) {
          ...
          return (
            <div style={STYLES.chart}>
              ...
              <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none meet" style={{ display: 'block', minHeight: 70 }}>
                <polyline
                  fill="none"
                  stroke="var(--accent)"
                  strokeWidth="1.5"
        ...
        const minV = allY.length ? Math.min(0, ...allY) : 0
        const maxV = allY.length ? Math.max(0, ...allY) : 1
          ...
          <div style={STYLES.chart}>
            ...
            <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none meet" style={{ display: 'block', minHeight: 70 }}>
              {slotKeys.map((key, idx) => {
                ...
                const points = pts
                  .map((p) => {
                    const x = pad + (p.x / divisor) * (w - 2 * pad)
                    const y = h - pad - ((p.y - minV) / range) * (h - 2 * pad)
                    return `${x},${y}`
                  })
                  .join(' ')
                return <polyline key={key} fill="none" stroke={color} strokeWidth="1.5" points={points} />
              })}
            </svg>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem 1rem', marginTop: '0.5rem', fontSize: '0.75rem' }}>
              {slotKeys.map((key, idx) => (
                <span key={key} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                  <span style={{ width: 8, height: 2, background: chartColors[idx % chartColors.length], borderRadius: 1 }} />
                  {slotNames[key] || key}
                </span>
              ))}
            </div>
          ...
          <div style={STYLES.chart}>
            ...
            <div style={STYLES.betList}>
              {[...betHistory].reverse().slice(0, 30).map((b) => {
                ...
                const cc = (currencyCode || targetCurrency)?.toUpperCase() || ''
                return (
                  <div
                    key={b.id}
                    style={{
                  ...
                  >
                    ...
                    <span>{formatAmount(b.betAmount, targetCurrency)}{cc ? ` ${cc}` : ''}</span>
                    <span style={{ color: b.winAmount > 0 ? 'var(--success)' : undefined }}>
                      {b.isBonus ? 'Bonus' : `${formatAmount(b.winAmount, targetCurrency)}${cc ? ` ${cc}` : ''}`}
                    </span>
                    <span style={{ color: net >= 0 ? 'var(--success)' : 'var(--error)' }}>
                      {b.isBonus ? '–' : `${net >= 0 ? '+' : ''}${formatAmount(net, targetCurrency)}${cc ? ` ${cc}` : ''}`}
                    </span>
